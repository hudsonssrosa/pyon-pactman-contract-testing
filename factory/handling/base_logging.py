...
from factory.utils.StringsUtil import StringUtil as StrUtil
...
class BaseLogging:
    ...
    def response_body(response):
        try:
            ...
            print(Color.blue(StrUtil.indent_dict_body(parsed_body)))
        except Exception as ex:
            message = BaseLogging.show_exception(
    ...
    @staticmethod
    def set_response_message(response_status, message):
        if response_status is None:
            return f"Assertion) {message}"
        else:
            return f"Assertion) Status {str(response_status)}: {message}"
